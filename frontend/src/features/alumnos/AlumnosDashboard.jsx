...
export default function AlumnosDashboard() {
  ...
  // El alumno SOLO puede reservar Cubículo (el backend ya lo restringe por rol).
  const tiposAlumno = tipos.filter((t) => t.nombre_tipo === 'Cubículo')
  const tipoObj = tiposAlumno.find((t) => t.tipo_id === tipoSel)
  ...
  useEffect(() => { if (tiposAlumno.length && tipoSel == null) setTipoSel(tiposAlumno[0].tipo_id) }, [tiposAlumno, tipoSel])
  // Al elegir día en el paso 2, consulta las horas ocupadas del cubículo (auto-asignado).
  useEffect(() => {
    if (paso === 2 && diaSel && tipoSel != null) {
      setHoraSlot(null)
      dispatch(fetchHorasOcupadas({ fecha: aISO(diaSel), tipoId: tipoSel }))
    }
  }, [paso, diaSel, tipoSel, dispatch])

  const diasConReserva = useMemo(() => new Set(mias.map((r) => r.fecha_reserva)), [mias])
  const nombre = perfilData?.nombre || user?.nombre || 'Alumno'
  const apellido = perfilData?.apellido || user?.apellido || ''
  const email = perfilData?.email || user?.email || 'alumno@upa.edu.mx'
  const iniciales = `${nombre[0] || 'A'}${apellido[0] || ''}`.toUpperCase()

  const paso1Listo = tipoSel != null
  ...
  const handleReservar = async () => {
    const res = await dispatch(crearReserva({
      tipo_id: tipoSel, // Cubículo: la BD auto-asigna un espacio libre.
      titulo: titulo.trim() || 'Reserva de cubículo',
      fecha: aISO(diaSel),
    ...
    if (crearReserva.fulfilled.match(res)) {
      ...
      setPaso(1); setDiaSel(null); setHoraSlot(null); setTitulo('')
    }
  }
    ...
    <div className="flex min-h-screen flex-col bg-[#F8FAFC] text-[#0F172A]">
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 px-6 py-8 lg:flex-row">
        <main className="min-w-0 flex-1">
          <p className="font-mono-upa text-[11px] font-medium uppercase tracking-[0.22em] text-slate-500">Estudiante</p>
          <h1 className="font-display text-4xl font-extrabold tracking-tight text-[#0F172A]">Nueva reserva</h1>
          <p className="mt-1 text-[15px] text-[#475569]">Reserva un cubículo de estudio en la biblioteca siguiendo estos pasos.</p>
          ...
          <div className="mt-6 flex flex-col overflow-hidden rounded-xl border border-[#E2E8F0] bg-white sm:flex-row">
            <Paso n="1" titulo="Qué reservar" sub="Tipo de espacio" estado={paso === 1 ? 'activo' : 'hecho'} onClick={paso > 1 ? () => setPaso(1) : null} />
            ...
            <div className="border-t border-[#E2E8F0] sm:border-l sm:border-t-0" />
            <Paso n="3" titulo="Confirmación" sub="Resumen final" estado={paso === 3 ? 'activo' : 'pendiente'} onClick={null} />
          </div>
          ...
          {paso === 1 && (
            <section className="mt-6">
              ...
              <p className="mb-4 text-sm text-[#475569]">Como estudiante puedes reservar un cubículo de estudio.</p>
                ...
                {tiposAlumno.map((t) => {
                  ...
                  return (
                    <button key={t.tipo_id} onClick={() => setTipoSel(t.tipo_id)}
                      className={`relative flex items-start gap-3 rounded-xl border p-4 text-left transition ${activo ? 'border-upa-blue bg-upa-light/60 ring-1 ring-upa-blue' : 'border-[#E2E8F0] bg-white hover:border-upa-blue/50'}`}>
                      <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-lg ${activo ? 'bg-upa-blue text-white' : 'bg-slate-100 text-slate-500'}`}><IconCube /></span>
                      <span className="min-w-0 flex-1">
                        <span className="block font-display text-base font-bold text-[#0F172A]">{t.nombre_tipo}</span>
                        <span className="block text-sm text-[#475569]">{t.descripcion}</span>
                        <span className="mt-1 block font-mono-upa text-[10px] uppercase tracking-wide text-slate-500">Cap. {t.capacidad_default} personas</span>
                      </span>
                      {activo && (<span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-upa-blue text-white"><IconCheck /></span>)}
                    </button>
                  )
                })}
              </div>

              {/* Cubículo: la encargada asigna uno disponible al confirmar. */}
              <div className="mt-4 rounded-xl border border-upa-light bg-upa-light/40 p-4">
                ...
                <p className="mt-1 text-sm text-[#475569]">No necesitas elegir un cubículo específico: la encargada te asignará uno disponible al confirmar tu reserva.</p>
              </div>
          ...
          {paso === 2 && (
            <section className="mt-6">
              ...
              <p className="mb-4 text-sm text-[#475569]">Cubículo (Biblioteca) · selecciona un día disponible y una hora libre.</p>
            ...
            <section className="mt-6">
              ...
              <div className="max-w-md rounded-2xl border border-[#E2E8F0] bg-white p-5">
                <div className="mb-3">
                  ...
                  <input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Reserva de cubículo"
                ...
                <dl className="space-y-2 rounded-xl bg-[#F8FAFC] p-4 text-sm">
                  ...
                  <Fila k="Rol" v={user?.rol || 'Estudiante'} />
                  <Fila k="Tipo de espacio" v="Cubículo" />
                  <Fila k="Edificio" v="Biblioteca" />
                  <Fila k="Espacio" v="Lo asigna la encargada" />
                  <Fila k="Horario" v={`${aISO(diaSel)} · ${fmtHora(horaSlot)}–${fmtHora(horaSlot + 1)}`} />
                  <Fila k="Capacidad" v={`${tipoObj?.capacidad_default} personas`} />
                </dl>
        ...
        <aside className="w-full shrink-0 lg:w-72">
          <div className="rounded-xl border border-[#E2E8F0] bg-white p-4">
            ...
            <span className="mt-3 inline-block rounded-md bg-upa-light px-2 py-1 font-mono-upa text-[10px] font-semibold uppercase tracking-wide text-upa-blue">Estudiante</span>
          </div>
          ...
          <div className="max-h-[90vh] w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="max-h-[calc(90vh-160px)] overflow-y-auto px-5 py-4">
              ...
              <ol className="mt-4 list-decimal space-y-2.5 pl-5 text-sm text-[#475569]">
                <li>Llegar puntualmente al cubículo reservado. Tienes una tolerancia de <b>10 minutos</b>; pasado ese tiempo, la reserva se libera automáticamente.</li>
                <li>El cubículo debe utilizarse únicamente para fines académicos. Está prohibido consumir alimentos o bebidas dentro de los cubículos.</li>
                <li>El usuario es responsable del orden y los bienes dentro del cubículo durante el periodo reservado.</li>
